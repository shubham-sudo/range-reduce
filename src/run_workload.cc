...
int runWorkload(std::unique_ptr<DBEnv> &env) {
  ...
  while (std::getline(workload_file, line)) {
    ...
    case 'D': {
      ...
      // [ProbePointQuery]
    case 'P': { // for tectonic, point query is P insteat of Q
      std::string key, value;
      stream >> key;
    ...
    case 'S': {
      ...
      for (it->Seek(start_key); it->Valid(); it->Next()) {
        ...
        // std::cout << "Key: " << it->key().ToString() << " Value: " <<
      ...
      auto frag_score =
          (double)it->entries_skipped_ / it->entries_read_in_total_;
      std::cout << "entries_read_in_total: " << it->entries_read_in_total_
                << " entries_skipped: " << it->entries_skipped_ << "\n"
                << "fragmentation score = " << frag_score << std::endl
                << std::flush;
      if (frag_score > (1.0 / env->size_ratio) && env->rqc_enabled) {
        (*buffer) << "Fragmentation Score: " << frag_score << std::endl;
        (*buffer) << "Compacting range: [" << start_key << ", " << end_key
                  << "]" << std::endl;
        // std::thread([db, start_key, end_key]() {
        Slice s(start_key);
        Slice e(end_key);
        db->CompactRange(CompactRangeOptions(), &s, &e);
        // }).detach();
      }
    ...
    case 'R': {
      ...
      rdelete_exec_time += duration.count();
#endif // TIMER
      break;
    }
    default:
